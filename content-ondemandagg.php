...
//echo( get_the_ID() . "<br>");
$ishsdmentalityondemand = in_array( get_the_ID(), array(42671, 43828,44730 ) );

if( !$hasamemberproductaccess && $ishsdmentalityondemand )
    $canview = false;
...
if ( $canview || $hasamemberproductaccess ) // if they can view the video AND it's not the on demand video or they have access to the on demand video
...
else if( $api->isLoggedIn() )
	{
	    ...
	    if( $ishsdmentalityondemand )
	    	    $errorlink = '<a href="https://reports.hitsongsdeconstructed.com/members/signup/writing-producing-on-demand" target="_blank">Sign Up</a>  |  <a href="https://www.hitsongsdeconstructed.com/mastery-workshop-on-demand" target="_blank">Learn More</a>  '; 
	    else
...
<?php// if ( has_term( 'online-courses', 'ondemand_categories' ) ) : ?>
...
if($prop_agg_image!=''){ ?>
  ...
  <a href="<?php the_permalink(); ?>"><img src="<?=get_post_meta( get_the_ID(), 'display_image', true );?>" /></a>
    
<?php }else{ ?>
    	<a href="<?php the_permalink(); ?>"><?php the_post_thumbnail( array(515,377) ); ?></a>
    <?php } ?>
